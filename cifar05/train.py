...
# Learning loop
    ...
    while True:
        ...
        finalAccuracy = sum_accuracy/N_ev
        print sm.GetInfo()
...
if __name__=="__main__":
    ao = AmebaOptimizer.AmabaOptimizer()
    ao.SetParam("N_PLayers"  ,[2,3,4,5,6,7],initial=4)
    ao.SetParam("P0C_feature",[8,16,32,64] ,initial=32)
    ao.SetParam("P1C_feature",[8,16,32,64] ,initial=32)
    ao.SetParam("P2C_feature",[8,16,32,64] ,initial=16)
    ao.SetParam("P0C_filter" ,[3,5,7]      ,initial=3)
    ao.SetParam("P1C_filter" ,[3,5,7]      ,initial=3)
    ao.SetParam("P2C_filter" ,[3,5,7]      ,initial=3)
    ao.SetParam("P0P_ksize"  ,[1,2,3,4]    ,initial=3)
    ao.SetParam("P1P_ksize"  ,[1,2,3,4]    ,initial=2)
    ao.SetParam("P2P_ksize"  ,[1,2,3,4]    ,initial=2)
    ao.SetParam("L1_dropout" ,[0.5]        ,initial=0.5)
    ao.SetParam("L2_dropout" ,[0.0]        ,initial=0.0)
    ao.SetParam("L2_unit"    ,[50,100,200,500,1000] ,initial=200)

    f = open("AOoutput.dat","w")

    while True:
        BatchSize = 1000
        IterCount = 0
        param = ao.GetNextParam()
        if param==None: break
        ...
        while True:
            ...
            if IterCount>=10:
                ...
                break
            try:
                result = CifarAnalysis("Output/%s"%param,batchsize = BatchSize,**param)
                ao.SetResult(result)
                f.write("%.3f, %s\n"%(result,param))
                f.flush()
                print "Maximum result so far:",ao.GetMaxParamSet()
                print
                break
            except:
                print "Segmentation fault... Retry"
                BatchSize /= 2
    f.close()
