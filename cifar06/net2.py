...
class ImageProcessNetwork(chainer.Chain):
    ...
    def __call__(self, x):
        h = x
        h = self.__dict__["P1_1"](F.relu(h))
        h = self.__dict__["P1_2"](F.relu(h))
        h = F.max_pooling_2d(F.relu(h), ksize=3, stride=2, cover_all=False)
        h = self.__dict__["P2_1"](h)
        h = self.__dict__["P2_2"](F.relu(h))
        h = self.__dict__["P2_2"](F.relu(h))
        h = F.max_pooling_2d(F.relu(h), ksize=3, stride=2, cover_all=False)
        h = self.__dict__["P3_1"](h)
        h = self.__dict__["P3_2"](F.relu(h))
        h = self.__dict__["P3_3"](F.relu(h))
        h = F.average_pooling_2d(F.relu(h), ksize=6)
