...
class MnistMLP(chainer.Chain):
	...
	"""
        super(MnistMLP, self).__init__()
	self.add_link("l1",L.Linear(n_in, n_units))
	self.add_link("l2",L.Linear(n_units, n_units))
	self.add_link("l3",L.Linear(n_units, n_out))
...
class MnistMLPParallel(chainer.Chain):
	...
	"""
        super(MnistMLPParallel, self).__init__()
        self.add_link("first0" ,MnistMLP(n_in, n_units // 2, n_units).to_gpu(0))
        self.add_link("first1" ,MnistMLP(n_in, n_units // 2, n_units).to_gpu(1))
        self.add_link("second0",MnistMLP(n_units, n_units // 2, n_out).to_gpu(0))
        self.add_link("second1",MnistMLP(n_units, n_units // 2, n_out).to_gpu(1))
